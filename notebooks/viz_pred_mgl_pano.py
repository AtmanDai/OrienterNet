...
data_dir: "datasets/MGL_new"
loading:
    val: {batch_size: 4, num_workers: 0} # Batch size should be num_views multiples for panoramas
    train: ${.val}
add_map_mask: true
return_gps: true
""")))
...
experiment = "OrienterNet_MGL_ft_washington" 
...
out_dir = Path('viz_kitti/washinton_pano/fixed')
...
num_panoramas_to_viz = 12
num_panoramas_total = len(val_dataset) // 4
...
for i in range(num_panoramas_to_viz):
    ...
    batch_list = [val_dataset[start_idx], val_dataset[start_idx+1], val_dataset[start_idx+2], val_dataset[start_idx+3]]
